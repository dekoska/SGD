...
def runGame():
    ...
    while True: # game loop
        ...
        DISPLAYSURF.fill(BGCOLOR)
        drawBoard(board)
        drawStatus(score, level, lines_cleared)  
...
def drawStatus(score, level, lines_cleared):
    # draw the score text
    ...
    lines_cleared_surf = BASICFONT.render('Lines Cleared: %s' % lines_cleared, True, TEXTCOLOR)
    lines_cleared_rect = lines_cleared_surf.get_rect()
    lines_cleared_rect.topleft = (WINDOWWIDTH - 150, 80)
    DISPLAYSURF.blit(lines_cleared_surf, lines_cleared_rect)
