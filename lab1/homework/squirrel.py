...
def runGame():
    ...
    showLevelUp = False       # whether to show level up message
    levelUpStartTime = 0      # when level up message started showing
    speedMultiplier = 1.0     # current speed multiplier
    lastSpeedIncrease = 0     # last score when speed was increased
    ...
    levelUpSurf = BASICFONT.render('LEVEL UP!', True, RED)
    ...
    while True: # main game loop
        ...
        for sObj in squirrelObjs:
            ...
            sObj['x'] += sObj['movex'] * speedMultiplier
            sObj['y'] += sObj['movey'] * speedMultiplier
            sObj['bounce'] += 1
        ...
        scoreSurf = BASICFONT.render('Score: ' + str(score), True, RED)
        scoreRect = scoreSurf.get_rect()
        scoreRect.topleft = (50, 15)  
            ...
            for i in range(len(squirrelObjs)-1, -1, -1):
                ...
                if 'rect' in sqObj and playerObj['rect'].colliderect(sqObj['rect']):
                    ...
                    if sqObj['width'] * sqObj['height'] <= playerObj['size']**2:
                        ...
                        if score % 5 == 0 and score > lastSpeedIncrease:
                            speedMultiplier += 0.2 
                            lastSpeedIncrease = score
                            showLevelUp = True
                            levelUpStartTime = time.time()
                            levelUpSurf = BASICFONT.render('SPEED UP!', True, RED)
                        elif score > oldScore: 
                            showLevelUp = True
                            levelUpStartTime = time.time()
                            levelUpSurf = BASICFONT.render('LEVEL UP!', True, RED)
