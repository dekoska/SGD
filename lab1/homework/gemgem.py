...
def main():
    global FPSCLOCK, DISPLAYSURF, GEMIMAGES, GAMESOUNDS, BASICFONT, BOARDRECTS, GAMEOVERFONT
    ...
    GAMEOVERFONT = pygame.font.Font('freesansbold.ttf', GAMEOVERFONTSIZE)
...
def runGame():
    ...
    while True: # main game loop
        time_remaining = max(0, GAME_TIME_LIMIT - int(time.time() - start_time))
        if time_remaining == 0 and not gameIsOver:
            gameIsOver = True
        ...
        if gameIsOver:
            if clickContinueTextSurf == None:
                ...
                if time_remaining == 0:
                    message = 'Time\'s Up! Final Score: %s (Click to continue)' % (score)
                else:
                    message = 'Final Score: %s (Click to continue)' % (score)
                clickContinueTextSurf = GAMEOVERFONT.render(message, 1, GAMEOVERCOLOR, GAMEOVERBGCOLOR)
        ...
        timerSurf = BASICFONT.render('Time: %02d:%02d' % (minutes, seconds), 1, TIMER_COLOR)
