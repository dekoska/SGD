...
def runGame(isFirstGame):
    ...
    while True:
        ...
        drawBoard(mainBoard, turn=None)  # No turn text during game over
...
def drawBoard(board, extraToken=None, turn=None):
    ...
    if turn == HUMAN:
        ...
        text_rect.center = (WINDOWWIDTH // 2, 30)
            ...
            if event.type == QUIT:
                ...
                if tokeny < YMARGIN and tokenx > XMARGIN and tokenx < WINDOWWIDTH - XMARGIN:
                    ...
                    if isValidMove(board, column):
                        ...
                        drawBoard(board, turn=HUMAN)  # Pass turn information
                        pygame.display.update()
                        return
                tokenx, tokeny = None, None
                draggingToken = False
        if tokenx != None and tokeny != None:
            drawBoard(board, {'x':tokenx - int(SPACESIZE / 2), 'y':tokeny - int(SPACESIZE / 2), 'color':RED}, turn=HUMAN)  # Pass turn information
        else:
            drawBoard(board, turn=HUMAN)  # Pass turn information
...
def animateDroppingToken(board, column, color):
    ...
    while True:
        ...
        if int((y - YMARGIN) / SPACESIZE) >= lowestEmptySpace:
            return
        drawBoard(board, {'x':x, 'y':y, 'color':color}, turn=HUMAN if color == RED else COMPUTER)
        pygame.display.update()
        FPSCLOCK.tick()
